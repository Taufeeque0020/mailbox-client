...
export const composeMail = (mail, message) => {
  return async (dispatch) => {
    ...
    const sent = await axios.post(
      `https://mail-box-client-91486-default-rtdb.firebaseio.com/${short}.json`,
      {
    ...
    await axios.post(
      `https://mail-box-client-91486-default-rtdb.firebaseio.com/${shorts}.json`,
      {
  ...
  return async (dispatch) => {
    const readingData = async () => {
      ...
      const sent = await axios.put(
        `https://mail-box-client-91486-default-rtdb.firebaseio.com/${short}/${data.id}/inbox.json`,
        {
          From: data.From,
  ...
  return async (dispatch) => {
    const fetchData = async () => {
      ...
      const response = await axios.get(
        `https://mail-box-client-91486-default-rtdb.firebaseio.com/${mail}.json`
      );
...
export const deleteMessage = (id) => {
  return async (dispatch) => {
    ...
    const response = await axios.delete(
      `https://mail-box-client-91486-default-rtdb.firebaseio.com/${mail}/${id}.json`
    );
